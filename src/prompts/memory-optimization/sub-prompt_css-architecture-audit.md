# 📋 **SUB-PROMPT** — CSS Architecture & Accessibility Audit

**Role:** Sub-prompt called by orchestrator (CSS architecture, BEM, WordPress alignment, accessibility)

---

## **1) Orchestrator Instructions (Run Once Per Repository)**

* **Value:** This sub-prompt provides a **CSS architecture & accessibility auditor** with **smaller targeted audit prompts (A-J)** plus a **per‑project refactor prompt template** to restructure CSS (BEM + WordPress-aligned utilities + token-driven light/dark), with accessibility (WCAG 2.1 AA/AAA) treated as a hard requirement.  
...
* **Next step:** Run the **CSS Architecture Auditor** prompt below once **per repo** (or per Figma Make project). It will generate `/reports/audits/<date>_<repo>_css-audit.md` first, then (and only then) create `/reports/<date>_<repo>_task-list.md`.

---

## **2) CSS Architecture & Accessibility Auditor Prompt**
...
2) Do NOT produce a task list until AFTER you have successfully written an audit report into /reports/audits/.
...
B) Write a single consolidated Markdown report file:
   /reports/audits/<YYYY-MM-DD>_<repo-or-project-slug>_css-audit.md
...
## **3) Smaller Audit Prompts (A–J)**

These are the "smaller prompts" referenced above. You can run them manually (one by one) if you prefer, or let the orchestrator run them in sequence.
...
## **5) Best-Practice Architecture Recommendations**

These are the practical "rules of thumb" to bake into the audit + refactor decisions:

1. **Token-first, semantic-second**  
...
2. **Use cascade layers to prevent specificity wars**  
...
3. **Keep selectors low-specificity**  
...
4. **BEM + small utility set beats "everything utility"**  
...
5. **Theme switching: one mechanism**  
...
6. **"Per block" CSS like WordPress**  
