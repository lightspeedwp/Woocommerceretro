# 📋 **SUB-PROMPT** — Memory Optimisation for WooCommerce Demo

**Role:** Sub-prompt called by orchestrator (Figma & codebase memory optimization)
...
### **3. Clean up layers, variants and assets**
     
- Delete hidden or duplicated layers; hidden layers still consume memory[[5]](https://www.linkedin.com/posts/empathybilal_how-much-memory-is-my-figma-file-consuming-activity-7157964367454519298-TRSu#:~:text=off%20when%20you%27re%20done,%E2%9E%8C).  
      
- Flatten complex SVGs and vector shapes to reduce the number of nodes[[6]](https://www.linkedin.com/posts/empathybilal_how-much-memory-is-my-figma-file-consuming-activity-7157964367454519298-TRSu#:~:text=layers%20you%20don%27t%20need%20and,Replace%20variants%20with). Use the vector boolean or flatten selection tool in Figma before importing.  
      
- Trim the number of component variants; use Boolean properties instead of dozens of variants[[7]](https://www.linkedin.com/posts/empathybilal_how-much-memory-is-my-figma-file-consuming-activity-7157964367454519298-TRSu#:~:text=memory%20use,For%20more%20information%20from%20Figma). This also simplifies the generated React components.  
      
- Compress images before importing. Use PNG or JPG only at required dimensions; prefer WebP for photographs. Within the code, store images in `/src/app/imports/images/`, name them descriptively and include dimensions (e.g., `hero-banner-1920x1080.jpg`) and compress them as recommended[[8]](https://github.com/lightspeedwp/Woocommercedemo/blob/390fcc6d210b41fea0ec965ddb0e02cb59ed0808/src/guidelines/IMPORTS_GUIDELINES.md#L512-L518). Avoid loading the same image multiple times; create a wrapper component that centralises image imports.  
      
- Consolidate fonts and icons. Use only the required font weights (e.g., inter-400 and inter-700) and import via CSS @font-face in `/src/styles/` as per the import guidelines. For SVG icons, import the path data via relative paths (without .svg) and render via `<svg><path d={path}/></svg>`[[9]](https://github.com/lightspeedwp/Woocommercedemo/blob/390fcc6d210b41fea0ec965ddb0e02cb59ed0808/src/guidelines/IMPORTS_GUIDELINES.md#L196-L225).  
      
### **4. Refactor components and data for reuse**
...
```markdown
# Memory Optimization Audit — WooCommerce Demo — YYYY-MM-DD

## Executive Summary
- Current memory usage: X MB / 2048 MB (Y%)
- Pages approaching limit: [list]
- Top 5 memory consumers: [layers/components]
...
- Overall file size
- Per-page breakdown
- Layer memory consumption (top 20)

### 1.2 Problem Areas
- Hidden layers consuming memory
- Oversized images
...
### 2.1 Images
- Current total: X images, Y MB
- Compression opportunities
- Duplicate image usage
- Recommended dimensions

### 2.2 SVGs & Vectors
- Complex shapes to flatten
...
- Component name | Current size | Suggested splits

### 3.2 Repeated Patterns
- Pattern type | Occurrences | Consolidation opportunity

## 4. Code Structure Improvements
### 4.1 CSS Modularity
- Current globals.css size
...
- Components with embedded logic
- Recommended hooks/utils
